...
PATH_FACE = resource_path("data/face_img_database/")
# face recognition model, the object maps human faces into 128D vectors

datFile = resource_path("model/dlib_face_recognition_resnet_model_v1.dat")
facerec = dlib.face_recognition_model_v1(datFile)

detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(resource_path('model/shape_predictor_68_face_landmarks.dat'))
...
class WAS(wx.Frame):
    def __init__(self):
        wx.Frame.__init__(self, parent=None, title="Sistema de tarjeta perforada de monitoreo inteligente",
                          size=(1032, 560))
    ...
    def OnOpenLogcatClicked(self, event):
        ...
        self.SetSize(1340, 560)
        grid = wx.grid.Grid(self, pos=(420, 0), size=(900, 500))
        ...
        grid.SetColLabelValue(5, "A Tiempo")
        ...
        for i, id in enumerate(self.logcat_id):
            ...
            grid.SetCellValue(i, 5, self.logcat_lateSa[i])
    ...
    def SetWorkingHours(self, event):
        ...
        self.working_hours = wx.GetTextFromUser(message="Por favor, introduzca el horario de trabajo",
                                                caption="Consejos", default_value="07:00:00",
                                                parent=None)
    ...
    def SetOffWorkHours(self, event):
        ...
        self.offwork_hours = wx.GetTextFromUser(message="Por favor, introduzca la hora de cierre", caption="Consejos",
                                                default_value="16:00:00",
                                                parent=None)
        offworking = self.offwork_hours
        win32api.MessageBox(0,
                            "Asegúrese de configurar tanto el tiempo de encendido como el de apagado y configure el tiempo de encendido primero",
                            "recordar", win32con.MB_ICONWARNING)
        if setWorkingSign:
            self.loadDataBase(4)
        else:
    ...
    def ExportTodayLog(self, event):
        ...
        if not Folderpath1 == "":
            ...
            csv_writer.writerow(
                ["Numero de serie", "Nombre", "tiempo de entrada", "Estas tarde", "tiempo de salida", "Estas Temprano"])
            size = len(logcat_id)
            index = 0
            while size - 1 >= index:
                localtime1 = str(logcat_datetime[index]).replace('[', '').replace(']', '')
                localtime11 = str(logcat_datetimeSa[index]).replace('[', '').replace(']', '')
                csv_writer.writerow([logcat_id[index], logcat_name[index], localtime1, logcat_late[index], localtime11,
                                     logcat_lateSa[index]])
                index += 1;
            f.close()
        pass
    ...
    def OnClose(self, event):
        dlg = wx.MessageDialog(None, u'¿Está seguro de que desea cerrar esta ventana?', u'Consejos de operación',
                               wx.YES_NO)
    ...
    def OnYes(self, event):
        dlg = wx.MessageDialog(None, u'¿Está seguro de que desea eliminar al empleado con este número?',
                               u'Consejos de operación', wx.YES_NO)
        if dlg.ShowModal() == wx.ID_YES:
            return True
    ...
    def DoCustomLog(self, event):
        if not Folderpath2 == "":
            ...
            if flag:
                ...
                csv_writer = csv.writer(f)
                csv_writer.writerow(
                    ["Numero de serie", "Nombre", "tiempo de perforacion", "Estas tarde", "tiempo de salida",
                     "Temprano"])
                for index in range(row):
                    s1 = str(find_datetime[index]).replace('[', '').replace(']', '')
                    s11 = str(find_datetimeSa[index]).replace('[', '').replace(']', '')
                    csv_writer.writerow([str(find_id[index]), str(find_name[index]), s1, str(find_late[index]), s11,
                                         str(find_lateSa[index])])

                f.close()
                success = wx.MessageDialog(None, 'El registro se guardó correctamente, preste atención para verificar',
                                           'info', wx.OK)
                success.ShowModal()
            else:
                warn = wx.MessageDialog(None, 'La identificación de entrada es incorrecta, vuelva a ingresar', 'info',
                                        wx.OK)
                warn.ShowModal()
            dialog.Destroy()
        else:
            win32api.MessageBox(0, "Ingrese la ubicación de exportación del archivo", "recordar",
    ...
    def deleteById(self, event):
        ...
        if length <= 0:
            win32api.MessageBox(0, "No se encuentra el empleado, vuelva a ingresar la identificación", "recordar",
                                win32con.MB_ICONWARNING)
            return False
        else:
    ...
    def findById(self, id, day):
        global find_id, find_name, find_datetime, find_late, find_datetimeSa, find_lateSa
        find_id = []
        find_name = []
        find_datetime = []
        find_late = []
        ...
        conn = sqlite3.connect("inspurer.db")  # Establecer conexión con la base de datos
        cur = conn.cursor()  # obtener el objeto del cursor
        sql = 'select logcat.id,logcat.name,logcat.datetime,logcat.late,logsalida.datetimeSa,logsalida.lateSa from logcat left join logsalida on logcat.id = logsalida.id where logcat.id=' + id

        if day == '30':
            str = "'"
            sql1 = 'select logcat.id,logcat.name,logcat.datetime,logcat.late,logsalida.datetimeSa,logsalida.lateSa from logcat left join logsalida on logcat.id = logsalida.id where logcat.id=' + id + ' ' + 'and datetime like ' + str + '%' + last_month.strftime(
                "%Y-%m") + '%' + str

        else:
            sql1 = 'select logcat.id,logcat.name,logcat.datetime,logcat.late,logsalida.datetimeSa,logsalida.lateSa from logcat left join logsalida on logcat.id = logsalida.id where logcat.id=' + id + ' ' + 'and datetime>=' + day_before
        length = len(cur.execute(sql).fetchall())
        if length <= 0:
            return False
        else:
            ...
            for row in origin:
                ...
                find_datetimeSa.append(row[4])
                find_lateSa.append(row[5])
            return True
        pass
            ...
            if len(dets) != 0:
                    ...
                    if compare == "similar":  # encontrado caras similares
                        self.infoText.AppendText(self.getDateAndTime() + "Número de empleo:" + str(self.knew_id[i])
                                                 + " Nombre:" + self.knew_name[
                                                     i] + " de los datos faciales ya existe\r\n")
                ...
                try:
                    ...
                    if len(self.name) > 0:
                        ...
                        self.pic_num += 1
                        print("escribir a locales：",
                              str(PATH_FACE + self.name) + "/img_face_" + str(self.pic_num) + ".jpg")
    ...
    def punchcard_cap(self, event):
        ...
        if len(working_times) == 0:
            win32api.MessageBox(0,
                                "No ha configurado el tiempo de trabajo, configure el tiempo de trabajo primero y luego configure el tiempo de trabajo",
                                "recordar", win32con.MB_ICONWARNING)
                    ...
                    for i, knew_face_feature in enumerate(self.knew_face_feature):
                        ...
                        if compare == "similar":  # encontrado caras similares
                            ...
                            for j, logcat_name in enumerate(self.logcat_name):
                                if logcat_name == self.knew_name[i] and nowdt[0:nowdt.index(" ")] == \
                                    ...
                                    self.infoText.AppendText(nowdt + "Número de empleo:" + str(self.knew_id[i])
                                                             + " Nombre:" + self.knew_name[
                                                                 i] + " Error al iniciar sesión, inicio de sesión repetido\r\n")
                                    speak_info(
                                        self.knew_name[i] + "Error al iniciar sesión, inicio de sesión repetido ")
                                    flag = 1
                                    break
                            ...
                            if nowdt[nowdt.index(" ") + 1:-1] <= working:
                                ...
                                self.infoText.AppendText(nowdt + "Número de empleo:" + str(self.knew_id[i])
                                                         + " Nombre:" + self.knew_name[
                                                             i] + " Error al iniciar sesión, excediendo el tiempo de inicio de sesión\r\n")
                ...
                if len(dets) != 0:
                    ...
                    for i, knew_face_feature in enumerate(self.knew_face_feature):
                        ...
                        if compare == "similar":  # encontrado caras similares
                            ...
                            flag = 0
                            nowdt = self.getDateAndTime()
                            for j, logcat_nameSa in enumerate(self.logcat_nameSa):
                                if logcat_nameSa == self.knew_name[i] and nowdt[0:nowdt.index(" ")] == \
                                        self.logcat_datetimeSa[
                                            j][
                                        0:self.logcat_datetimeSa[
                                            j].index(" ")]:
                                    self.infoText.SetBackgroundColour((65, 225, 210))
                                    self.infoText.AppendText(nowdt + "Número de empleo:" + str(self.knew_id[i])
                                                             + " Nombre:" + self.knew_name[i] + " Ya salio\r\n")
                                    speak_info(self.knew_name[i] + "Ya salios ")
                                    flag = 1
                                    break
                            ...
                            if nowdt[nowdt.index(" ") + 1:-1] <= offworking:
                                self.infoText.SetBackgroundColour((191, 238, 30))
                                self.infoText.AppendText(nowdt + "Número de empleo:" + str(self.knew_id[i])
    ...
    def insertARow(self, Row, type):
        ...
        if type == 3:
            cur.execute("insert into logsalida (id,nameSa,datetimeSa,lateSa) values(?,?,?,?)",
                        (Row[0], Row[1], Row[2], Row[3]))
            print("registro de escritura exitoso")
            pass
        cur.close()
    ...
    def loadDataBase(self, type):
        ...
        print(day)
        global logcat_id, logcat_name, logcat_datetime, logcat_name, logcat_datetimeSa, logcat_lateSa, logcat_late, working_times, offworking_times
        conn = sqlite3.connect("inspurer.db")
        ...
        if type == 2:
            ...
            cur.execute(
                'select logcat.id,logcat.name,logcat.datetime,logcat.late,logsalida.nameSa,logsalida.datetimeSa,logsalida.lateSa from logcat left join logsalida on logcat.id = logsalida.id')
            origin = cur.fetchall()
            for row in origin:
                print(row[0])
                ...
                print(row[4])
                self.logcat_nameSa.append(row[4])
                print(row[5])
                self.logcat_datetimeSa.append(row[5])
                print(row[6])
                self.logcat_lateSa.append(row[6])
        ...
        if type == 3:
            ...
            logcat_lateSa = []
            s = "'"
            sql = 'select w.id,w.name,l.datetime,l.late, logsalida.datetimeSa,logsalida.lateSa from worker_info w left join logcat l on  w.id=l.id left join logsalida on w.id=logsalida.id and l.datetime like' + ' ' + s + day + '%' + s + ' ' + 'order by datetime desc'
            print(sql)
            cur.execute(sql)
            origin = cur.fetchall()
            for row in origin:
                ...
                print(row[4])
                logcat_datetimeSa.append(row[4])
                print(row[5])
                logcat_lateSa.append(row[5])
        if type == 4:
